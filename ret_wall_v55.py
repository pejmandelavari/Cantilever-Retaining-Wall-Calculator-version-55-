...
components.html(
    ...
    (function () {
      ...
      hint.innerHTML = "👈 <span style='margin-left:6px;'>Tap it to enter inputs</span>";

      // استایل مستقیم روی خود عنصر
      hint.style.position = "fixed";
      hint.style.top = "55px";              // این عدد رو برای بالاتر/پایین‌تر تغییر بده
      hint.style.left = "10px";
      hint.style.zIndex = "2147483647";     // حداکثر z-index (جلوی همه چیز)
      ...
      hint.style.gap = "6px";
      hint.style.pointerEvents = "none";   // مزاحم کلیک روی UI نشه
      ...
      // (اختیاری) بعد از 8 ثانیه محو شود
      setTimeout(() => {
    ...
    with st.sidebar:
        ...
        L_toe  = st.number_input("L_toe (m)", value=1.5, min_value=0.05, step=0.1)
        L_heel = st.number_input("L_heel (m)", value=3.2, min_value=0.05, step=0.1)

        st.divider()
        gamma_backfill = st.number_input("γ backfill (kN/m³)", value=20.0, min_value=1.0, step=0.5)
        phi = st.number_input("φ (deg)", value=30.0, min_value=0.0, max_value=60.0, step=0.5)
        delta = st.number_input("δ (deg)", value=10.0, min_value=0.0, max_value=45.0, step=0.5)
        beta = st.number_input("β (deg)", value=15.0, min_value=0.0, max_value=45.0, step=0.5)

        st.divider()
        gamma_concrete = st.number_input("γ concrete (kN/m³)", value=24.0, min_value=10.0, step=0.5)
        t_cover_front = st.number_input("t_cover (front soil) (m)", value=0.6, min_value=0.0, step=0.05)
